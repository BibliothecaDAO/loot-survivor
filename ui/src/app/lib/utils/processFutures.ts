...
function getBaseDiscoveryAmount(level: bigint, entropy: bigint): bigint {
  let discovery_multiplier = level / BigInt(5);
  let discovery_range = (discovery_multiplier + BigInt(1)) * BigInt(3);
  let discovery_amount = BigInt(1) + (entropy % discovery_range);

  return discovery_amount;
}
...
function getObstacleLevel(level: bigint, entropy: bigint): bigint {
  let obstacleLevel = BigInt(1) + (entropy % (level * BigInt(2)));
...
function getBeastHealth(level: bigint, seed: bigint): bigint {
  let health = BigInt(1) + (seed % (level * BigInt(15)));
...
function discoveryEncounter(
  ...
  let discovery_type = rnd2 % BigInt(2);

  let discovery_amount = getBaseDiscoveryAmount(level, rnd2);

  return {
    encounter: "Discovery",
    type: discovery_type === BigInt(0) ? "Gold" : "Health",
    tier: `${Number(discovery_amount)}`,
    nextXp: xp + 1,
  };
}
...
function calculateEncounterDamage(
  ...
  let crit_bonus = critMultiplier * 20;
